...
#import "core/resolve.typ": nonempty, _walk
...
#let calc-requirements(data) = {
  ...
  for r in reqs {
    ...
    let cost = float(r.at("qty", default: 1)) * float(r.at("unit_cost", default: 0))
...
#let find-orphans(data) = {
  ...
  let task-ids = {
    let gantt = data.at("baselines", default: (:)).at("schedule", default: (:)).at("gantt", default: (:))
    ...
    if type(tasks) != array { () }
      ...
      tasks.map(phase => {
        ...
        if type(subs) != array { () }
        else { subs.map(s => s.at("id", default: none)).filter(id => id != none) }
      }).flatten()
    }
  }
  ...
  let sc = data.at("initiation", default: (:)).at("success_criteria", default: ())
  if type(sc) == array {
    for c in sc {
      let oid = c.at("objective_id", default: none)
      if oid != none and oid not in objective-ids {
        push-orphan("success_criteria." + c.at("id", default: "?"), "objective_id", oid, "initiation.objectives")
      }
    }
  }

  // assumptions_log → risk_register
  let assumptions = data.at("initiation", default: (:)).at("assumptions_log", default: ())
  if type(assumptions) == array {
    for a in assumptions {
      let rid = a.at("risk_id", default: none)
      if rid != none and rid not in risk-ids {
        push-orphan("assumptions_log." + a.at("id", default: "?"), "risk_id", rid, "registers.risk_register")
      }
    }
  }

  // risk_register → tasks, milestones, assumptions
  let risks = data.at("registers", default: (:)).at("risk_register", default: ())
  if type(risks) == array {
    for r in risks {
      for tid in (if type(r.at("affects_wbs", default: ())) == str { (r.at("affects_wbs", default: ()),) } else { r.at("affects_wbs", default: ()) }) {
        if tid not in task-ids { push-orphan("risk_register." + r.at("id", default: "?"), "affects_wbs", tid, "baselines.schedule.gantt") }
      }
      for mid in (if type(r.at("blocks_milestone", default: ())) == str { (r.at("blocks_milestone", default: ()),) } else { r.at("blocks_milestone", default: ()) }) {
        if mid not in milestone-ids { push-orphan("risk_register." + r.at("id", default: "?"), "blocks_milestone", mid, "baselines.schedule.milestones") }
      }
      let sa = r.at("source_assumption", default: none)
      if sa != none and sa not in assumption-ids { push-orphan("risk_register." + r.at("id", default: "?"), "source_assumption", sa, "initiation.assumptions_log") }
    }
  }
  ...
  if type(issues) == array {
    for i in issues {
      for mid in (if type(i.at("blocks_milestone", default: ())) == str { (i.at("blocks_milestone", default: ()),) } else { i.at("blocks_milestone", default: ()) }) {
        if mid not in milestone-ids { push-orphan("issue_log." + i.at("id", default: "?"), "blocks_milestone", mid, "baselines.schedule.milestones") }
      }
      for did in (if type(i.at("blocks_deliverable", default: ())) == str { (i.at("blocks_deliverable", default: ()),) } else { i.at("blocks_deliverable", default: ()) }) {
        if did not in deliverable-ids { push-orphan("issue_log." + i.at("id", default: "?"), "blocks_deliverable", did, "registers.deliverables_register") }
      }
    }
  }

  // decision_log → risks, issues
  let decisions = data.at("registers", default: (:)).at("decision_log", default: ())
  if type(decisions) == array {
    for d in decisions {
      let pr = d.at("prompted_by_risk", default: none)
      if pr != none and pr not in risk-ids { push-orphan("decision_log." + d.at("id", default: "?"), "prompted_by_risk", pr, "registers.risk_register") }
      let pi = d.at("prompted_by_issue", default: none)
      if pi != none and pi not in issue-ids { push-orphan("decision_log." + d.at("id", default: "?"), "prompted_by_issue", pi, "registers.issue_log") }
    }
  }

  // budget line_items → requirements
  let budget = data.at("baselines", default: (:)).at("financials", default: (:)).at("budget", default: (:))
  if type(budget) == dictionary {
    for item in budget.at("line_items", default: ()) {
      let rid = item.at("req_id", default: none)
      if rid != none and rid not in req-ids { push-orphan("budget." + item.at("id", default: "?"), "req_id", rid, "baselines.requirements") }
    }
  }

  // deliverables → requirements
  for d in data.at("registers", default: (:)).at("deliverables_register", default: ()) {
    for rid in (if type(d.at("req_ids", default: ())) == str { (d.at("req_ids", default: ()),) } else { d.at("req_ids", default: ()) }) {
      if rid not in req-ids { push-orphan("deliverables_register." + d.at("id", default: "?"), "req_ids", rid, "baselines.requirements") }
    }
  }
  ...
  for a in data.at("closure", default: (:)).at("acceptance", default: ()) {
    ...
    if did != none and did not in deliverable-ids { push-orphan("acceptance", "deliverable_id", did, "registers.deliverables_register") }
  }
  ...
  for b in data.at("closure", default: (:)).at("benefits_review", default: ()) {
    ...
    if oid != none and oid not in objective-ids { push-orphan("benefits_review", "objective_id", oid, "initiation.objectives") }
  }
  ...
  if type(quality) == dictionary {
    for c in quality.at("criteria", default: ()) {
      ...
      if rid != none and rid not in req-ids { push-orphan("quality.criteria", "req_id", rid, "baselines.requirements") }
    }
  }

  // compliance → requirements, requirements → compliance
  for c in data.at("baselines", default: (:)).at("compliance", default: ()) {
    for rid in (if type(c.at("req_ids", default: ())) == str { (c.at("req_ids", default: ()),) } else { c.at("req_ids", default: ()) }) {
      if rid not in req-ids { push-orphan("compliance." + c.at("id", default: "?"), "req_ids", rid, "baselines.requirements") }
    }
  }
  for r in data.at("baselines", default: (:)).at("requirements", default: ()) {
    for cid in (if type(r.at("compliance_ids", default: ())) == str { (r.at("compliance_ids", default: ()),) } else { r.at("compliance_ids", default: ()) }) {
      if cid not in compliance-ids { push-orphan("requirements." + r.at("id", default: "?"), "compliance_ids", cid, "baselines.compliance") }
    }
  }
...
#let audit-missing(data, extra-checks: ()) = {
  ...
  full-schema.map(entry => {
    ...
    let status = if not r.found { "Missing" }
      else if not nonempty(data, entry.path) { "Empty" }
      else { "Present" }
    (path: entry.path, severity: entry.severity, phase: entry.at("phase", default: "custom"), kind: entry.at("kind", default: "unknown"), status: status)
  })
}

/// Count field statuses by severity.
#let audit-summary(audit-results) = {
  let count(sev, status) = audit-results.filter(r => r.severity == sev and r.status == status).len()
  (
    critical:    (present: count("critical",    "Present"), empty: count("critical",    "Empty"), missing: count("critical",    "Missing")),
    important:   (present: count("important",   "Present"), empty: count("important",   "Empty"), missing: count("important",   "Missing")),
    recommended: (present: count("recommended", "Present"), empty: count("recommended", "Empty"), missing: count("recommended", "Missing")),
  )
}
...
#let compute-context(data, config: (:)) = {
  ...
  (
    budget: budget, requirements: requirements, orphans: orphans,
