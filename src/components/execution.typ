...
#let change-log(data-path) = context {
  ...
  if type(changes) == array {
    data-table(
      ...
      headers: ("ID", "Change Description", "Type", "Affects Baseline", "Approval"),
      rows: changes.map(c => (
  ...
  if type(decisions) == array {
    data-table(
      ...
      rows: decisions.map(d => {
        ...
        if risk-id != none { body += [\ *Prompted by risk:* #link-to-risk(risk-id)] }
        if issue-id != none { body += [\ *Prompted by issue:* #link-to-issue(issue-id)] }
...
#let deliverables-register(data-path) = context {
  ...
  if type(deliverables) == array {
    data-table(
      ...
      headers: ("ID", "Description", "Owner", "Due Date", "Requirements", "Status"),
      rows: deliverables.map(d => {
        ...
        (
          ...
          badge(d.at("status", default: "Planned"), intent: if d.at("status", default: "") == "Accepted" { "success" } else if d.at("status", default: "") == "Rejected" { "danger" } else if d.at("status", default: "") == "In-Review" { "warning" } else { "neutral" })
        )
      }).flatten()
    )
  } else {
